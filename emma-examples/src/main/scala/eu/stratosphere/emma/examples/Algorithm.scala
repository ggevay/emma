...
abstract class Command[R, A <: Algorithm[R]](implicit val m: scala.reflect.Manifest[A]) {
   ...
   * @return
   */
  def instantiate(ns: Namespace): Algorithm[R] = {
    // instantiate runtime
    ...
    constructor.newInstance(ns, rt).asInstanceOf[Algorithm[R]]
  }
}
